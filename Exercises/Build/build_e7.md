...
### Preface: [Kubernetes](https://kubernetes.io/) is an __Orchestration__ platform. Up till now we have been "spinning up" and "spinning down" containers manually. One at a time we have been isolating individual processes and running them via a container but in an automated environment or production environment, we need a way to:

Run multiple containers as well as update them; 

Rotate them in and out of deployment; 

Keep track of them etc. etc. 

This exercise will help to familiarize with the core concepts of kubernetes (k8s)
...
```
- Now you can validate that it is working by issuing your first command.
```
# kubectl get nodes
...
## As you can see all functionality of the tool __kubectl__ can be accessed via the api natively, you just need to know the [api calls](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.10/#-strong-api-overview-strong-), in fact there are calls that you can only access outside of __kubectl__ but in general the "fat client" known as __kubectl__ will make your life easier not only as a developer, or as a defender, but also as an attacker. Therefore, red teamers and pen-testers should keep arch specific builds of the __kubectl__ binary available during engagements as they can be copied over to comramized containers and use for pivot attacks. (more on that later :-) )

## By default (and unless otherwise configured) internal DNS is handeled via the __kube-dns__ service. It will automatically setup name resolution for pods, services, and other objects created within the cluster. Lets create an example.
...
```
## See!  We now have 2 pods, both running the __httpbin__ container setup as a replica set, bound to a deployment, exposed via a service! Not bad!; With those two little commands (could be a single yaml file) we hve created a redundant yet deployable instruction set. If you want to view that, run a command like this:
```
