# Exercise #7 (k8s infrastructure basics: apiserver, dns, resource types, etc.)
...
### At it's heart, K8s has a few services that make up it's core services (Scheduler, Controlers, config storage [etcd] and API server) 
...
```
- Now you can valadate that it's working by issueing your first command.
```
# kubectl get nodes
...
## Lets explore. Because kubernetes requires authentacation we will not be able to just access the api server directly.
...
EOF
``` 
### Note: doing it this way will create an entire __deployment__ which is eisier to work with.
...
```
## see we have 2 pods, both running the __httpbin__ container setup as a replica set, bound to a deployment, exposed via a service! Not bad; with thoes two little commands (could be a single yaml file) we hve created a redundent yet deployable instruction set. If you want to view that, run a command like this:
```
# kubectl -n default get all 
```
## This new service __httpbin-svc__ can be used throughout the cluster via two dns records __"httpbin-svc"__ and it's F.Q.D.N. __"httpbin-svc.default.svc.cluster.local"__, again both recorads are created dynamicly via the __kube-dns__ service running in the cluster.
...
```
- then run it via minikube asa well
```
minikube dashboard
...
### click around and have a look at some of the features. You can view, edit, deploy, revert and many other actions withing the cluster.
...
# Review: 
## Now we have our single node kubernetes cluster up and running. We deployed a simple service into it and reviewed how to see it, both via the "fat client", "api" and Web Gui Dashboard. In the next exercise we will descuss tools that will make our life easier as an Admin, Developer, Security practisional and Pentester.
